...
function getApiBaseUrl() {
  ...
  const serverValue = process.env.BACKEND_API_URL?.trim() || process.env.NEXT_PUBLIC_API_URL?.trim()
  if (!serverValue) {
    throw new Error("BACKEND_API_URL or NEXT_PUBLIC_API_URL is not configured. Set it in the frontend environment.")
  }
...
function buildNetworkErrorMessage(apiUrl: string) {
  if (typeof window !== "undefined" && window.location.protocol === "https:" && getConfiguredBackendOrigin().startsWith("http://")) {
...
function normalizeUser(payload: Record<string, unknown>): User {
  ...
  return {
    ...
    firstName,
    lastName,
    name: [firstName, lastName].filter(Boolean).join(" ").trim() || String(payload.email || ""),
    role: String(payload.role || "user") as User["role"],
    isVerified: Boolean(payload.is_email_verified),
    isActive: payload.is_active === undefined ? undefined : Boolean(payload.is_active),
...
function normalizeTherapist(payload: Record<string, unknown>): Therapist {
  ...
  const qualifications = toArray(typeof payload.qualifications === "string" ? payload.qualifications : "")
  const languages = toArray(typeof payload.languages === "string" ? payload.languages : "")
  ...
  return {
    ...
    approvalStatus: String(payload.approval_status || "pending") as Therapist["approvalStatus"],
    licenseNumber: String(payload.license_number || ""),
    avatar: profileImage || undefined,
    profileImage: profileImage || undefined,
    clinic: payload.clinic ? normalizeTherapistClinic(payload.clinic as Record<string, unknown>) : null,
  }
}

function normalizeTherapistClinic(payload: Record<string, unknown>): TherapistClinic {
  return {
    id: String(payload.id || ""),
...
function buildTherapistProfilePayload(payload: Partial<Therapist> & { profileImageFile?: File | null }) {
...
function normalizeAvailability(payload: Record<string, unknown>, therapistId = ""): AvailabilitySlot {
  return {
    id: String(payload.id),
    therapistId,
...
function normalizeSubscription(payload: Record<string, unknown>): UserSubscription {
  return {
    id: String(payload.id || ""),
    plan: normalizePackagePlan((payload.plan as Record<string, unknown>) || {}),
    status: String(payload.status || "pending_payment") as UserSubscription["status"],
    ...
    paymentStatus: String(payload.payment_status || "unpaid") as UserSubscription["paymentStatus"],
...
function normalizeAppointment(payload: Record<string, unknown>): Appointment {
  return {
    ...
    availabilitySlotId: payload.availability_slot ? String(payload.availability_slot) : null,
    sessionType: String(payload.session_type || "video") as Appointment["sessionType"],
    ...
    paymentStatus: String(payload.payment_status || "unpaid") as Appointment["paymentStatus"],
    bookingPaymentType: String(payload.booking_payment_type || "single") as Appointment["bookingPaymentType"],
    subscriptionId: payload.subscription ? String(payload.subscription) : null,
    paymentProvider: String(payload.payment_provider || ""),
    paidAmount: Number(payload.paid_amount || 0),
    subscriptionCreditConsumedAt: String(payload.subscription_credit_consumed_at || ""),
    subscriptionCreditRestoredAt: String(payload.subscription_credit_restored_at || ""),
    khaltiPidx: String(payload.khalti_pidx || ""),
    paymentTransactionId: String(payload.payment_transaction_id || ""),
    paymentInitiatedAt: String(payload.payment_initiated_at || ""),
    paymentVerifiedAt: String(payload.payment_verified_at || ""),
    ...
    rescheduledFrom: payload.rescheduled_from ? String(payload.rescheduled_from) : null,
...
function normalizeSupportMessage(payload: Record<string, unknown>): SupportTicketMessage {
  return {
    id: String(payload.id || ""),
...
function normalizeSupportTicket(payload: Record<string, unknown>): SupportTicket {
  return {
    id: String(payload.id || ""),
    ...
    messages: Array.isArray(payload.messages)
      ? payload.messages.map((item) => normalizeSupportMessage((item as Record<string, unknown>) || {}))
      : [],
    createdAt: String(payload.created_at || ""),
    updatedAt: String(payload.updated_at || ""),
  }
...
function normalizeFeedback(payload: Record<string, unknown>): Feedback {
  return {
    ...
    appointmentScheduledStart: String(payload.appointment_scheduled_start || ""),
    user: String(payload.user || ""),
    userName: String(payload.user_name || ""),
...
function normalizePatientRecord(payload: Record<string, unknown>): PatientRecord {
  return {
    id: String(payload.id),
...
function normalizeAdminAction(payload: Record<string, unknown>): AdminAction {
  return {
    id: String(payload.id),
    adminId: String(payload.admin || ""),
...
function normalizeConversation(payload: Record<string, unknown>): Conversation {
  return {
    ...
    isActive: Boolean(payload.is_active),
    messages: Array.isArray(payload.messages)
      ? payload.messages.map((message) => normalizeMessage(message as Record<string, unknown>))
      : [],
    createdAt: String(payload.created_at || ""),
  }
}
...
class ApiService {
  ...
  private async request<T>(endpoint: string, options: RequestOptions = {}): Promise<T> {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        console.error("API request failed before receiving a response.", { apiUrl, error })
      }
    ...
    if (response.status === 401 && options.auth !== false && options.retryOnAuthError !== false) {
      const refreshed = await this.refreshTokens()
      if (refreshed?.access) {
        return this.request<T>(endpoint, { ...options, retryOnAuthError: false })
      }
    }
  ...
  async refreshTokens(): Promise<AuthTokens | null> {
    ...
    if (!this.refreshPromise) {
      ...
      this.refreshPromise = fetch(apiUrl, {
        ...
        .catch((error) => {
          if (process.env.NODE_ENV !== "production") {
            console.error("Token refresh request failed before receiving a response.", { apiUrl, error })
          }
          this.notifyUnauthorized()
          return null
        })
  ...
  async applyAsTherapist(payload: {
    ...
    const data = await this.request<Record<string, unknown>>("/therapists/apply/", {
      method: "POST",
      body,
      auth: false,
    })
  ...
  async verifyEmail(token: string): Promise<User> {
    const data = await this.request<Record<string, unknown>>("/auth/verify-email/", {
      method: "POST",
  ...
  async listTherapists(
    ...
    const data = await this.request<PaginatedResponse<Record<string, unknown>> | Record<string, unknown>[]>(
  ...
  async getTherapist(id: string, options: { auth?: boolean } = {}): Promise<Therapist> {
    const data = await this.request<Record<string, unknown>>(`/therapists/profiles/${id}/`, {
      auth: options.auth,
    })
    return normalizeTherapist(data)
  }
  ...
  async getMyTherapistProfile(): Promise<Therapist> {
    const data = await this.request<Record<string, unknown>>("/therapists/profiles/me/")
    return normalizeTherapist(data)
  }

  async updateMyTherapistProfile(payload: Partial<Therapist> & { profileImageFile?: File | null }): Promise<Therapist> {
    const data = await this.request<Record<string, unknown>>("/therapists/profiles/me/", {
      method: "PATCH",
  ...
  async getMyTherapistClinic(): Promise<TherapistClinic | null> {
    const data = await this.request<Record<string, unknown> | null>("/therapists/profiles/me/clinic/")
  ...
  async saveMyTherapistClinic(payload: {
    ...
    const data = await this.request<Record<string, unknown>>("/therapists/profiles/me/clinic/", {
      method: "PUT",
      body: JSON.stringify({
  ...
  async approveTherapist(id: string, approvalStatus: "approved" | "rejected"): Promise<Therapist> {
    const data = await this.request<Record<string, unknown>>(`/therapists/profiles/${id}/approve/`, {
      method: "POST",
  ...
  async getAvailability(therapistId?: string): Promise<AvailabilitySlot[]> {
    ...
    const results = Array.isArray(data) ? data : data.results
    return results.map((item) => normalizeAvailability(item, therapistId || ""))
  }

  async createAvailability(startTime: string, endTime: string): Promise<AvailabilitySlot> {
    const data = await this.request<Record<string, unknown>>("/therapists/availability/", {
      method: "POST",
  ...
  async updateAvailability(id: string, startTime: string, endTime: string): Promise<AvailabilitySlot> {
    const data = await this.request<Record<string, unknown>>(`/therapists/availability/${id}/`, {
      method: "PATCH",
      body: JSON.stringify({ start_time: startTime, end_time: endTime }),
    })
    return normalizeAvailability(data)
  ...
  async getAppointment(id: string): Promise<Appointment> {
    const data = await this.request<Record<string, unknown>>(`/appointments/${id}/`)
  ...
  async initiateKhaltiPayment(appointmentId: string): Promise<KhaltiInitiation> {
    const frontendOrigin = typeof window !== "undefined" ? window.location.origin : undefined
    const data = await this.request<Record<string, unknown>>("/payments/khalti/initiate/", {
      method: "POST",
      body: JSON.stringify({ appointment: appointmentId, frontend_origin: frontendOrigin }),
    })
    return {
  ...
  async verifyKhaltiPayment(
    ...
    const data = await this.request<Record<string, unknown>>("/payments/khalti/verify/", {
      method: "POST",
      body: JSON.stringify({
    ...
    return {
      appointment: data.appointment ? normalizeAppointment((data.appointment as Record<string, unknown>) || {}) : null,
      appointmentId: String(data.appointment_id || payload.appointment || ""),
      paymentStatus: String(data.payment_status || "unpaid") as KhaltiVerificationResult["paymentStatus"],
      bookingStatus: String(data.booking_status || "pending_payment") as KhaltiVerificationResult["bookingStatus"],
      lookup: (data.lookup as Record<string, unknown>) || {},
    }
  }

  async cancelAppointment(id: string, reason = ""): Promise<Appointment> {
    const data = await this.request<Record<string, unknown>>(`/appointments/${id}/cancel/`, {
      method: "POST",
  ...
  async acceptAppointment(id: string, note = ""): Promise<Appointment> {
    const data = await this.request<Record<string, unknown>>(`/appointments/${id}/accept/`, {
      method: "POST",
  ...
  async rejectAppointment(id: string, note = ""): Promise<Appointment> {
    const data = await this.request<Record<string, unknown>>(`/appointments/${id}/reject/`, {
      method: "POST",
      body: JSON.stringify({ note }),
    })
    return normalizeAppointment(data)
  ...
  async completeAppointment(
    ...
    const data = await this.request<Record<string, unknown>>(`/appointments/${id}/complete/`, {
      method: "POST",
      body: JSON.stringify({
        notes: payload.notes,
  ...
  async getPackagePlans(options: { auth?: boolean } = { auth: false }): Promise<PackagePlan[]> {
    const data = await this.request<PaginatedResponse<Record<string, unknown>> | Record<string, unknown>[]>(
      "/packages/plans/",
      { auth: options.auth }
    )
  ...
  async createPackagePlan(payload: PackagePlanInput): Promise<PackagePlan> {
    const data = await this.request<Record<string, unknown>>("/packages/plans/", {
      method: "POST",
      body: JSON.stringify({
        name: payload.name,
        slug: payload.slug,
  ...
  async updatePackagePlan(id: string, payload: Partial<PackagePlanInput>): Promise<PackagePlan> {
    const body: Record<string, unknown> = {}
    if (payload.name !== undefined) body.name = payload.name
    if (payload.slug !== undefined) body.slug = payload.slug
    if (payload.description !== undefined) body.description = payload.description
    if (payload.sessionCredits !== undefined) body.session_credits = payload.sessionCredits
    if (payload.durationDays !== undefined) body.duration_days = payload.durationDays
    if (payload.priceAmount !== undefined) body.price_amount = payload.priceAmount
    if (payload.isActive !== undefined) body.is_active = payload.isActive

    const data = await this.request<Record<string, unknown>>(`/packages/plans/${id}/`, {
      method: "PATCH",
  ...
  async getMySubscriptions(): Promise<UserSubscription[]> {
    const data = await this.request<Record<string, unknown>[] | PaginatedResponse<Record<string, unknown>>>(
  ...
  async purchasePackagePlan(planId: string): Promise<PackagePurchaseInitiation> {
    const frontendOrigin = typeof window !== "undefined" ? window.location.origin : undefined
    const data = await this.request<Record<string, unknown>>(`/packages/plans/${planId}/purchase/`, {
      method: "POST",
  ...
  async verifyPackagePayment(
    ...
    const data = await this.request<Record<string, unknown>>("/packages/subscriptions/verify/", {
      method: "POST",
      body: JSON.stringify({
    ...
    return {
      subscription: data.subscription ? normalizeSubscription((data.subscription as Record<string, unknown>) || {}) : null,
      subscriptionId: String(data.subscription_id || payload.subscription || ""),
      paymentStatus: String(data.payment_status || "unpaid") as PackageVerificationResult["paymentStatus"],
      subscriptionStatus: String(data.subscription_status || "pending_payment") as PackageVerificationResult["subscriptionStatus"],
      lookup: (data.lookup as Record<string, unknown>) || {},
    }
  }
  ...
  async getResource(id: string, options: { auth?: boolean } = {}): Promise<Resource> {
    const data = await this.request<Record<string, unknown>>(`/resources/${id}/`, { auth: options.auth })
    return normalizeResource(data)
  }

  async createCategory(payload: { name: string; description?: string }): Promise<ResourceCategory> {
    const data = await this.request<Record<string, unknown>>("/resources/categories/", {
      method: "POST",
      body: JSON.stringify({
        name: payload.name,
        description: payload.description || "",
  ...
  async updateCategory(id: string, payload: { name: string; description?: string }): Promise<ResourceCategory> {
    const data = await this.request<Record<string, unknown>>(`/resources/categories/${id}/`, {
      method: "PATCH",
      body: JSON.stringify({
        name: payload.name,
        description: payload.description || "",
  ...
  async updateResource(
    ...
    const data = await this.request<Record<string, unknown>>(`/resources/${id}/`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    })
    return normalizeResource(data)
  ...
  async markNotificationRead(id: string): Promise<Notification> {
    const data = await this.request<Record<string, unknown>>(`/notifications/${id}/mark_read/`, {
      method: "POST",
    })
    return normalizeNotification(data)
  }

  async getSupportTickets(filters?: { status?: string; issueType?: string }): Promise<SupportTicket[]> {
    const params = new URLSearchParams()
  ...
  async getSupportTicket(id: string): Promise<SupportTicket> {
    const data = await this.request<Record<string, unknown>>(`/support/tickets/${id}/`)
  ...
  async createSupportTicket(payload: {
    ...
    const data = await this.request<Record<string, unknown>>("/support/tickets/", {
      method: "POST",
      body: JSON.stringify({
  ...
  async updateSupportTicketStatus(id: string, statusValue: SupportTicketStatus): Promise<SupportTicket> {
    const data = await this.request<Record<string, unknown>>(`/support/tickets/${id}/`, {
      method: "PATCH",
  ...
  async sendSupportMessage(ticketId: string, message: string): Promise<SupportTicketMessage> {
    const data = await this.request<Record<string, unknown>>(`/support/tickets/${ticketId}/messages/`, {
      method: "POST",
  ...
  async submitFeedback(appointment: string, rating: number, comment: string): Promise<Feedback> {
    const data = await this.request<Record<string, unknown>>("/feedback/", {
      method: "POST",
      body: JSON.stringify({ appointment, rating, comment }),
    })
    return normalizeFeedback(data)
  }

  async getFeedback(filters?: { date?: string; therapistId?: string }, options: { auth?: boolean } = {}): Promise<Feedback[]> {
    const params = new URLSearchParams()
    if (filters?.date) params.set("date", filters.date)
  ...
  async getPatientRecords(filters?: { patientId?: string }): Promise<PatientRecord[]> {
    const params = new URLSearchParams()
  ...
  async createPatientRecord(payload: {
    ...
    const data = await this.request<Record<string, unknown>>("/patient-records/", {
      method: "POST",
      body: JSON.stringify({
  ...
  async updatePatientRecord(
    ...
    const data = await this.request<Record<string, unknown>>(`/patient-records/${id}/`, {
      method: "PATCH",
      body: JSON.stringify({
        patient: payload.patient,
        appointment: payload.appointment || null,
  ...
  async getAdminUsers(filters?: { role?: string; isActive?: boolean; date?: string }): Promise<User[]> {
    const params = new URLSearchParams()
  ...
  async getAdminUser(id: string): Promise<User> {
    const data = await this.request<Record<string, unknown>>(`/auth/admin/users/${id}/`)
    return normalizeUser(data)
  }

  async updateAdminUser(id: string, payload: Partial<User> & { isActive?: boolean }): Promise<User> {
    const data = await this.request<Record<string, unknown>>(`/auth/admin/users/${id}/`, {
      method: "PATCH",
      body: JSON.stringify({
        first_name: payload.firstName,
  ...
  async activateAdminUser(id: string): Promise<User> {
    const data = await this.request<Record<string, unknown>>(`/auth/admin/users/${id}/activate/`, {
      method: "POST",
    })
    return normalizeUser(data)
  }
  ...
  async deactivateAdminUser(id: string): Promise<User> {
    const data = await this.request<Record<string, unknown>>(`/auth/admin/users/${id}/deactivate/`, {
      method: "POST",
    })
    return normalizeUser(data)
  }

  async getAdminActions(filters?: { date?: string }): Promise<AdminAction[]> {
    const params = new URLSearchParams()
    if (filters?.date) params.set("date", filters.date)
    const data = await this.request<PaginatedResponse<Record<string, unknown>> | Record<string, unknown>[]>(
      `/admin-actions/${params.toString() ? `?${params.toString()}` : ""}`
    )
    const results = Array.isArray(data) ? data : data.results
    return results.map((item) => normalizeAdminAction(item))
  }
  ...
  async createConversation(appointment: string, therapist: string): Promise<Conversation> {
    const data = await this.request<Record<string, unknown>>("/communications/conversations/", {
      method: "POST",
  ...
  async sendMessage(conversation: string, content: string): Promise<Message> {
    const data = await this.request<Record<string, unknown>>("/communications/messages/", {
      method: "POST",
  ...
  async chatWithRecommendations(
    message: string,
    options?: { conversationContext?: AIChatMessagePayload[]; userContext?: Record<string, unknown> }
  ): Promise<AIChatReply> {
    const data = await this.request<Record<string, unknown>>("/ai/chat/", {
      method: "POST",
      body: JSON.stringify({
